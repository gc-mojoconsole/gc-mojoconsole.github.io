...
function genQuest() {
    ...
    <div style="border: 1px solid gray; border-radius: 5px; padding: 1em;">
            <label for="quest-search">Quest List:</label>
                <div style="display: flex; flex-direction: column;">
                    <div style="display: flex; align-items: center; overflow: hidden;">
                        <input id="quest-search" style="flex: 4" type="text" placeholder="Search Quest Name" />
                    ...
                    <div>
                        ...
                        <label class="checkable" for="check-hidden">Show "HIDDEN"</label>
                        <input id="check-test" type="checkbox" name="test" onchange="updateQuestList()" />
                        <label class="checkable" for="check-test">Show "(test)"</label>
                        <input id="check-unrelease" type="checkbox" name="test" onchange="updateQuestList()" />
                        <label class="checkable" for="check-unrelease">Show "UNRELEASE"</label>
                    </div>
            ...
            <div style="margin-top: 2em; border: 1px solid gray; border-radius: 5px; margin-top: 1em; padding: 1em; " >
                <label for="step-list" >Sub-quest:</label>
...
function updateSubQuestList(){
    ...
    quest_list[quest_name].forEach(id_desc => {
        ...
        var id = id_desc[0];
        var desc = id_desc[1] ? id_desc[1] : "MISSING";
        var content = `
            <div style="padding-left: 0.2em;">
                <div style="color: green; font-style: italic; font-size: 0.8em;">${quest_name} Step: ${current}/${total}</div>
                <div style="margin-left: 1em; font-size: 1em;">${desc}</div>
            </div>
            <div>
                <button class="success" quest-id="${id}" class="">Add</button>
                <button class="warning" quest-id="${id}" class="">Finish</button>
            </div>
        `
        o.innerHTML = content;
